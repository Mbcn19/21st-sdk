...
import { isDonationDomainAllowed } from "@/lib/donation"
...
export async function PATCH(req: NextRequest) {
  try {
    let userId = (await getAgentsRequestSession(req)).internalUserId

    if (!userId) {
      userId = await verifyDesktopAuth(req)
    }
    ...
    if (donation_url) {
      try {
        const parsed = new URL(
          donation_url.startsWith("http")
            ? donation_url
            : `https://${donation_url}`,
        )
        if (!isDonationDomainAllowed(parsed.hostname)) {
          return NextResponse.json(
            { error: "Donation URL domain not allowed" },
            { status: 400 },
          )
        }
      } catch {
    ...
    if (donation_url !== undefined) {
      updateData.donation_url = donation_url ? processUrl(donation_url) : null
    }
