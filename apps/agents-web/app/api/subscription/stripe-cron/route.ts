...
/**
 * Resets usage for users with yearly plans and free users (no active plans) at the beginning of the month
 */
async function resetAllUsages() {
  try {
    // Update usage for users with yearly plans and free users (no active plans or inactive plans)
    const result = await prisma.usage.updateMany({
      where: {
        usage: { gt: 0 }, // Only update records that need to be reset
        OR: [
          // Users with active yearly plans
          {
            user: {
              plansInfo: {
                status: "active",
          ...
          // Free users (no plan at all)
          {
            user: {
              plansInfo: null,
            },
          },
          // Free users (inactive plan)
          {
            user: {
              plansInfo: {
                status: { not: "active" },
    ...
    return {
      success: true,
      message: `Usage reset for ${result.count} users with yearly plans and free users`,
    }
  } catch (error) {
    console.error(
      "Error resetting usages for yearly plans and free users:",
      error,
    )
    throw error
  }
