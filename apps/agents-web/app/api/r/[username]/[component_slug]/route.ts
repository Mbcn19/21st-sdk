...
// Security: List of dangerous dependencies that can execute arbitrary commands
const DANGEROUS_DEPENDENCIES = [
  ...
  "exec-webpack-plugin",
  "command-webpack-plugin",
  "script-webpack-plugin",
  "esbuild-plugin-shell",
  "rollup-plugin-shell",
...
// Security: List of critical config files that should never be overwritten
...
function validateRegistryResponse(response: ComponentRegistryResponse): {
  isValid: boolean
  errors: string[]
} {
  const errors: string[] = []

  // Check dependencies for dangerous packages
  if (response.dependencies) {
    const dangerousDeps = response.dependencies.filter((dep) =>
      DANGEROUS_DEPENDENCIES.some((dangerous) =>
        dep.toLowerCase().includes(dangerous.toLowerCase()),
      ),
    )
    if (dangerousDeps.length > 0) {
      errors.push(
        `Dangerous dependencies detected: ${dangerousDeps.join(", ")}`,
      )
    }
  }

  // Check file paths for config file overwrites
  if (response.files) {
    for (const file of response.files) {
      const fileName = file.path.split("/").pop()?.toLowerCase() || ""
      const fullPath = file.path.toLowerCase()

      // Check if trying to overwrite protected config files
      if (
        PROTECTED_CONFIG_FILES.some(
          (protectedFile) =>
            fileName === protectedFile.toLowerCase() ||
            fullPath.includes(protectedFile.toLowerCase()),
        )
      ) {
        errors.push(`Attempt to overwrite protected config file: ${file.path}`)
      }

      // Check for directory traversal attacks
      if (file.path.includes("..") || file.path.includes("~")) {
        errors.push(`Directory traversal detected in file path: ${file.path}`)
      }

      // Check for script tags or executable content
      if (
        file.content.includes("<script") ||
        file.content.includes("javascript:") ||
        file.content.includes("vbscript:")
      ) {
        errors.push(`Script content detected in file: ${file.path}`)
      }
    }
  }

  // Check CSS vars for injection attempts
  if (response.cssVars) {
    const cssVarsStr = JSON.stringify(response.cssVars)
    if (
      cssVarsStr.includes("javascript:") ||
      cssVarsStr.includes("expression(") ||
      cssVarsStr.includes("@import") ||
      cssVarsStr.includes("url(data:")
    ) {
      errors.push("Potential CSS injection detected in cssVars")
    }
  }
...
function logSecurityIncident(
  ...
  userAgent?: string,
  ipAddress?: string,
  registrySource: "internal" | "external" = "internal",
) {
  // Log security incident to console for monitoring
  const incidentData = {
    incident_type: "registry_injection_attempt",
  ...
  console.warn(
    `🚨 SECURITY ALERT - Registry injection attempt detected:`,
    incidentData,
  )

  // TODO: Set up proper security incident logging table and endpoint for admin monitoring
}
...
export async function GET(
  ...
  try {
    ...
    if (component.sandbox_id && component.registry_url) {
      try {
        ...
        if (registryRes.ok) {
          ...
          // Security: Validate external registry content
          const validationResult = validateRegistryResponse(registryJson)
          if (!validationResult.isValid) {
            ...
            console.warn(
              "External registry content failed security validation:",
              validationResult.errors,
            )
            return NextResponse.json(
              {
                error: "External registry content failed security validation",
    ...
    const validationResult = validateRegistryResponse(responseData)
    if (!validationResult.isValid) {
      const userAgent = request.headers.get("user-agent") || undefined
      const ipAddress =
        request.headers.get("x-forwarded-for") ||
      ...
      console.warn(
        "Registry response failed security validation:",
        validationResult.errors,
      )
      return NextResponse.json(
        {
          error: "Registry content failed security validation",
          details: validationResult.errors,
        },
        { status: 400 },
      )
