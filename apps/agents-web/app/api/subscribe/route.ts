...
const AUDIENCES = {
  ...
  SDK_EARLY_ACCESS: process.env.SDK_EARLY_ACCESS_AUDIENCE ?? "",
  AN_EARLY_ACCESS: "00a47180-9bcb-4e71-9596-2cc0029169fa",
} as const
...
export async function POST(request: Request) {
  try {
    ...
    if (
      !type ||
      !["newsletter", "magic-waitlist", "magic-chat", "sdk-early-access", "an-early-access"].includes(type)
    ) {
      return NextResponse.json(
    ...
    const audienceId =
      type === "newsletter"
        ...
        : type === "magic-waitlist"
          ...
          : type === "sdk-early-access"
            ? AUDIENCES.SDK_EARLY_ACCESS
            : type === "an-early-access"
              ? AUDIENCES.AN_EARLY_ACCESS
