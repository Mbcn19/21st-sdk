...
export async function POST(request: NextRequest) {
  ...
  // Check Clerk auth first - logged in users don't need API key
  const { userId } = await auth()
  const isAuthenticated = !!userId

  // Require either: logged in user, internal request, or valid API key
  if (!isAuthenticated && !isInternalRequest && !apiKey) {
    ...
    const { data: demo, error: demoError } = await supabase
      ...
      .select(
        ...
        *,
        ...
        user:users!demos_user_id_fkey(username)
      `,
      )
      .eq("id", demo_id)
      .single()
