...
export async function addToAudience(
  email: string,
  type: "newsletter" | "magic-waitlist" | "sdk-early-access" | "an-early-access",
) {
  try {
...
export async function addToSdkEarlyAccess(email: string) {
  return addToAudience(email, "sdk-early-access")
}

export async function addToAnEarlyAccess(email: string) {
  return addToAudience(email, "an-early-access")
}
