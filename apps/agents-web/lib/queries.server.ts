...
> {
  ...
  const r2FetchPromises = dependencies.map(async (dep) => {
    ...
    try {
        ...
        if (stylesPromises.length > 0) {
          try {
            ...
            styles = {
              ...
              globalCss: global_css_extension
                ? texts[texts.length - 1]
                : undefined,
            }
          } catch (styleError) {
            console.error(
              `Error fetching styles for ${username}/${component_slug}:`,
              styleError,
            )
          }
        }
      ...
      console.error(
        `Error fetching component ${username}/${component_slug}:`,
        error,
      )
      return {
        error: new Error(
          `Error fetching component ${username}/${component_slug}: ${error instanceof Error ? error.message : String(error)}`,
        ),
        npmDependencies: npmDependencies,
        fileWithRegistry: null,
        styles: null,
