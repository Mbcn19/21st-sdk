...
import { promptCachingMiddleware } from "./prompt-caching-middleware"
...
export const fulfillModel = (
  ...
  if (model.startsWith("gemini-")) {
    ...
    const xaiToOpenRouterMap = {
      "grok-4": "x-ai/grok-4",
    }

    result = {
      model: openrouter(
        xaiToOpenRouterMap[model as keyof typeof xaiToOpenRouterMap],
      ),
    }
  } else if (model.startsWith("gpt-")) {
    const openaiToOpenRouterMap = {
      "gpt-5": "openai/gpt-5",
    ...
    result = {
      model: openrouter(
        openaiToOpenRouterMap[model as keyof typeof openaiToOpenRouterMap],
      ),
    }
...
async function geminiGenerateText(
  ...
  const { model } = fulfillModel("gemini-2.5-flash", false)
...
import * as magicPrompts from "../ai/magic.prompt"
