...
const Message = ({ children, className, role, ...props }: MessageProps) => (
  <div
    className={cn(
      ...
      role === "user" ? "justify-end" : "flex-row gap-3",
      className,
    )}
    {...props}
  >
...
const MessageContent = ({
  ...
  const baseClasses = cn(
    "text-foreground text-sm prose break-words whitespace-normal max-w-none",
    role === "user"
      ? "rounded-md px-4 py-2 bg-foreground/10 ml-auto"
      : "text-left",
    className,
  )
