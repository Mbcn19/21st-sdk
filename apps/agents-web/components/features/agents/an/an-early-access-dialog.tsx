...
import { addToAnEarlyAccess } from "@/lib/resend"
import { toast } from "sonner"
import { Mail, X } from "lucide-react"

const EASING_CURVE = [0.55, 0.055, 0.675, 0.19] as const

export function AnEarlyAccessDialog({
  open,
  onOpenChange,
}: {
  open: boolean
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { success, error: apiError } = await addToAnEarlyAccess(email)
                  ...
                  <form onSubmit={handleSubmit} className="mt-5 space-y-3">
                    <div className="relative">
                      <input
                        ...
                        className={`w-full h-9 rounded-[10px] bg-white/[0.05] border pl-9 pr-4 text-sm text-white placeholder:text-white/25 outline-none transition-colors ${error ? "border-red-500/50 focus:border-red-500/70" : "border-white/[0.08] focus:border-white/20"}`}
                      />
                    ...
                    <button
                      ...
                      className="w-full inline-flex items-center justify-center rounded-[10px] h-8 px-4 text-sm font-medium bg-white text-[#09090b] hover:bg-white/90 shadow-[0_0_0_0.5px_rgba(255,255,255,0.5),inset_0_0_0_1px_rgba(255,255,255,0.14)] transition-colors duration-150 disabled:opacity-50"
                    >
