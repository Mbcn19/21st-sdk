...
export function SearchGroupRich({
  ...
  const { displayResults, sourceTabs } = useMemo(() => {
    ...
    if (firstSource && SEARCH_RESULT_SETS[firstSource]) {
      ...
      return { displayResults: set.results.map((r) => ({ ...r, url: undefined as string | undefined })), sourceTabs: set.tabs }
    }
          ...
          <div className="max-h-[200px] overflow-y-auto">
            <div className="flex flex-col gap-px p-1">
              {displayResults.map((result, i) => {
                ...
                return result.url ? (
                  <a key={i} href={result.url} target="_blank" rel="noopener noreferrer" className="no-underline">
                    {inner}
                  </a>
                ) : (
                  <div key={i}>{inner}</div>
